...
def plot_moving_averages(df, ma_data):
    ...
    print("Length of ma20:", len(ma_data['ma20']))
    print("Length of ma50:", len(ma_data['ma50']))
    print("Length of ma200:", len(ma_data['ma200']))

    if not all(len(ma_data[key]) == len(df) for key in ['ma20', 'ma50', 'ma200']):
        raise ValueError("Moving averages do not align with the DataFrame length. Ensure sufficient data points.")
...
def plot_rsi(rsi_data, df):
    """Plot the RSI using mplfinance."""
    validate_dataframe(df)  # Ensure the DataFrame is valid for mplfinance

    # Validate RSI data
    if len(rsi_data) != len(df):
        raise ValueError("RSI data length must match the DataFrame length.")

    # Create additional plots for RSI and thresholds
    additional_plots = [
        mpf.make_addplot(rsi_data, color='purple', panel=1, ylabel='RSI'),  # RSI values
        mpf.make_addplot([70] * len(df), color='red', linestyle='--', panel=1, label='Overbought'),  # Overbought line
        mpf.make_addplot([30] * len(df), color='green', linestyle='--', panel=1, label='Oversold')   # Oversold line
    ]
    
    # Plot the chart
    mpf.plot(df, type='candle', addplot=additional_plots, title='Relative Strength Index (RSI)', style='charles', panel_ratios=(2, 1))
